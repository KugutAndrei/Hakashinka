...
def generateConfigFile(
    ...
    if add_filename is not None:
        elems.append(Element('additional-files', value=add_filename))
    inp.extend(elems)
...
class AditionalFile:
    ...
    def setVehicleFlow(self, calID, vtype, dur):
        if calID in self.flows.keys():
            self.flows[calID].append({
                'begin' : "0",
                'end' : str(int(dur)),
                'type' : vtype
            })
    ...
    def save(self):
        ...
        self.top.append(Comment("Calibrators"))
        elems = []
        for ID, pars in self.calibrs.items():
            elem = Element('calibrator', pars)
            if ID in self.flows:
                elems_s = []
                for pars_s in self.flows[ID]:
                    elems_s.append(Element('flow', pars_s))
                elem.extend(elems_s)
            elems.append(elem)
        self.top.extend(elems)
